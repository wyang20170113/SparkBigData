...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val head = StructField(columnNames(0),StringType,nullable = true)
    val tail = columnNames.tail.map(fieldName => StructField(fieldName,DoubleType))
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val primary = columnNames.filter(col => col.startsWith("t01")):::
                  columnNames.filter(col => col.startsWith("t03")):::
                  columnNames.filter(col => col.startsWith("t11")):::
                  columnNames.filter(col => col.startsWith("t1801")):::
                  columnNames.filter(col => col.startsWith("t1803"))
    val working = columnNames.filter(col => col.startsWith("t05")):::
                  columnNames.filter(col => col.startsWith("t1805"))
    val other = columnNames.diff(primary::working)
    (primary.map(col=> new Column(col)),working.map(col => new Column(col)),other.map(col=> new Column(col)))
  }
  ...
  def timeUsageSummary(
    ...
    def sum_udf(columns: List[Column], sum: Column): Column = columns match {
      case Nil => sum
      case x::xs => sum_udf(xs, sum + df(x.toString())/60.0)
    }
    val primaryNeedsProjection: Column = (sum_udf(primaryNeedsColumns, lit(0.0d))).as("primaryNeeds")
    val workProjection: Column = (sum_udf(workColumns, lit(0.0d))).as("work")
    val otherProjection: Column = (sum_udf(otherColumns, lit(0.0d))).as("other")
    df
