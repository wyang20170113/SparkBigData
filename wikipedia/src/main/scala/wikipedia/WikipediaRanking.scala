...
object WikipediaRanking {
  ...
  val conf: SparkConf = new SparkConf().setMaster("local[*]").setAppName("Wikipedia APP")//.set("spark.driver.host", "localhost")
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd.cache()
    rdd.filter(wa => wa.mentionsLanguage(lang)).count().toInt
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = {
    //sc.parallelize(langs.map(lang => (lang,rdd.filter(x => x.mentionsLanguage(lang)).collect().toIterable)))
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    index.map(a => (a._1, a._2.toList.length)).collect().toList
  }
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    langs.map(lang => (lang,rdd.filter(wa => wa.mentionsLanguage(lang)).count.toInt))
    /*rdd.flatMap(article => {
